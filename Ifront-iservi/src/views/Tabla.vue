...
<v-container>
  ...
  >
          ...
          <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn
                color="blue darken-1"
                text
                @click="editarCustomer(editedItem)"
              >
                Modificar
              </v-btn>
            </v-card-actions>
    ...
    <template v-slot:item.actions="{ item }">
      ...
      <v-icon
        small
        @click="eliminarCustomer(item._id)"
      >
        mdi-delete
      </v-icon>
    </template>
  ...
  export default {
    data: () => ({
      ...
      headers: [
        ...
        { text: 'Identificación Cliente', value: 'id_customer' },
        { text: 'Nombre Cliente', value: 'name' },
        { text: 'Correo Electrónico', value: 'email' },
        { text: 'Dirección', value: 'address' },
        { text: 'Nombre usuario', value: 'username' },
        { text: 'Contraseña', value: 'password' },
        { text: 'Usuario Activo', value: 'active_user' },
      ...
      editarCustomer(item){
       ...
       .then(res=>{
        ...
        this.$swal('success!',
         ...
         this.desserts[index].id_costumer=res.data.id_costumer;
         this.desserts[index].name=res.data.name;
         this.desserts[index].email=res.data.email;
         this.desserts[index].address=res.data.address;
         this.desserts[index].username=res.data.username;
         this.desserts[index].password=res.data.password;
       }).catch(e=>{
